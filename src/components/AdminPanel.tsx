...
export default function AdminPanel({
  ...
  // Turn image upload into base64 (with multi-image support: 1st is main, rest are extra)
  const handleImageUploaded = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      mainReader.onloadend = () => {
        setUploadedBase64(mainReader.result as string);
      };
      mainReader.readAsDataURL(firstFile);

      if (files.length > 1) {
        const extraFiles = Array.from(files).slice(1);
        const promises = extraFiles.map((file: any) => {
        ...
        Promise.all(promises).then((base64s) => {
          const currentUrls = extraImagesText.trim()
            ? extraImagesText.split(/[\n,]+/).map(url => url.trim())
            : [];
          const updatedUrls = [...currentUrls, ...base64s];
          setExtraImagesText(updatedUrls.join('\n'));
        });
      }
    }
  };

  const handleExtraImagesUploaded = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = e.target.files;
    if (files && files.length > 0) {
        ...
        <div className="bg-white dark:bg-neutral-900 p-6 rounded-2xl border border-neutral-200 dark:border-neutral-800 shadow-xs h-fit space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-xs font-semibold text-neutral-400 uppercase tracking-wider mb-1">Calidad del Producto (1-5)</label>
                <select
                  value={newScore}
                  onChange={(e) => setNewScore(parseInt(e.target.value))}
                  className="w-full px-3 py-2 bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl text-neutral-900 dark:text-white focus:ring-2 focus:ring-indigo-500 text-sm"
                >
                  <option value={5}>5 - Premium / Como nuevo ⭐⭐⭐⭐⭐</option>
                  <option value={4}>4 - Excelente estado ⭐⭐⭐⭐</option>
                  <option value={3}>3 - Buen estado ⭐⭐⭐</option>
                  <option value={2}>2 - Estado aceptable ⭐⭐</option>
                  <option value={1}>1 - Saldo / Para repuesto ⭐</option>
                </select>
              </div>
            </div>
              ...
              {imageType === 'url' ? (
                ...
                <div className="space-y-2">
                  ...
                  <span className="text-[10px] text-neutral-400 block mt-0.5 leading-tight">
                    Puedes seleccionar <strong>múltiples fotos</strong> a la vez. La primera se usará como portada y el resto se añadirá como imágenes adicionales.
                  </span>
                  {uploadedBase64 && (
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="block text-xs font-semibold text-neutral-400 uppercase tracking-wider flex items-center gap-1">
                ...
                <div>
                  <button
                    ...
                    className="flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/20 hover:bg-indigo-100 border border-indigo-200 dark:border-indigo-900 rounded-lg cursor-pointer transition-colors"
                  >
                    <Upload className="h-2.5 w-2.5" /> Subir fotos
                  </button>
                  <input
                    type="file"
                    ...
                    accept="image/*"
                    multiple
                    onChange={handleExtraImagesUploaded}
                    className="hidden"
                  />
                </div>
              </div>
              ...
              <span className="text-[10px] text-neutral-400 block mt-0.5 leading-relaxed">
                Agrega fotos adicionales para que el comprador las vea en un carrusel interactivo táctil. Se abrirán en tamaño grande al tocarlas. ¡Puedes presionar <strong>"Subir fotos"</strong> para cargar archivos!
              </span>
            </div>
            ...
            <div>
              <label className="block text-xs font-semibold text-neutral-400 uppercase tracking-wider mb-1 flex items-center gap-1">
                <Link className="h-3.5 w-3.5 text-indigo-500" /> Link al Producto Original (Opcional)
              </label>
              <input
                type="text"
                placeholder="Ej. https://amazon.com/... o tienda oficial del artículo"
                value={originalUrl}
              ...
              />
              <span className="text-[10px] text-neutral-400 block mt-0.5 leading-relaxed">
                Permite a los interesados ver el precio de venta original o la ficha técnica del producto real para comparar y valorar el descuento.
              </span>
            </div>
            ...
            <div className="flex items-center gap-2 pt-2">
              ...
              <label htmlFor="isOfferBonusCheckbox" className="text-xs text-neutral-600 dark:text-neutral-400 font-medium">
                Destacar como precio regalado / barato (Calidad ≤ 2 por defecto)
              </label>
            </div>
                      ...
                      <td className="py-3 px-2 text-right">
                        <button
                          onClick={() => {
                            ...
                            setOriginalUrl(product.originalUrl || '');
